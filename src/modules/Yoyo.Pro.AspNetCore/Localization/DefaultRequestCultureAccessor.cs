...
{
    public class DefaultRequestCultureAccessor : IRequestCultureAccessor
    {
        ...
        protected readonly ILogger<DefaultRequestCultureAccessor> _logger;

        public virtual RequestLocalizationOptions Options { get; set; }

        public DefaultRequestCultureAccessor(ILanguageManager languageManager, ILogger<DefaultRequestCultureAccessor> logger)
        {
        ...
        public virtual async Task<ProviderCultureResult> GetUserRequestCulture(HttpContext httpContext)
        {
            ...
            if (result == null || !result.Cultures.Any())
            {
                var defaultCulture = await this.GetDefaultRequestCulture(httpContext);
                if (defaultCulture != null && defaultCulture.Cultures.Any())
                {
                    culture = defaultCulture.Cultures.First().Value;
                    cultureWithUI = defaultCulture.UICultures.First().Value;

                    _logger.LogDebug("{0} {1} - Read from header", nameof(GetUserRequestCulture), nameof(GetDefaultRequestCulture));
                    _logger.LogDebug("Using Culture:{0} , UICulture:{1}", culture, cultureWithUI);
        ...
        public virtual Task<ProviderCultureResult> GetHeaderRequestCulture(HttpContext httpContext)
        {
            ...
            var localizationHeader = httpContext.Request.GetTypedHeaders().AcceptLanguage;
            ...
            _logger.LogDebug("{0} - Using Culture:{1} , UICulture:{2}", nameof(DefaultRequestCultureAccessor), culture, uiCulture);
        ...
        public virtual async Task<ProviderCultureResult> GetDefaultRequestCulture(HttpContext httpContext)
        {
            ...
            var defaultLanguage = languageManager.GetActiveLanguages().FirstOrDefault(o => o.IsDefault);
