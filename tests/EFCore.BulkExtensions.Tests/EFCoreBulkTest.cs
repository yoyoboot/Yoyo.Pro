...
namespace EFCore.BulkExtensions.Tests
{
    public class EFCoreBulkTest
    {
       ...
       // private static Func<TestContext, int> ItemsCountQuery = EF.CompileQuery<TestContext, int>(ctx => ctx.Items.Count());
        ...
        [InlineData(DbServerType.Oracle, true)]
        //[InlineData(DbServerType.SQLServer, false)] // for speed comparison with Regular EF CUD operations
        public async Task OperationsOracleAsnyc(DbServerType dbServer, bool isBulk)
        {
            ContextUtil.DbServer = dbServer;
        ...
        private async Task RunInsertAsync(bool isBulk)
        {
            ...
            context.BulkUpdate(entities1);

            var dfs = context.Product.ToList();

            await context.BulkDeleteAsync(dfs);
        }
        ...
        private void RunInsert(bool isBulk)
        {
            ...
            for (int i = 1; i <= 500; i++)
            {
                var entity = new Documents
                {
                    ...
                    Tag = "tianxia" + i,
                    ContentLength = 1,
                    //CreatorUserName = "dfcdfwe",
                    //DeleterUserNameXiaoTianTian = "fde",
                    //LastModificationTime = DateTime.Now,
                    //IsDeleted = false,
                    //LastModifierUserName = "xiaotian1"
                };
                entities1.Add(entity);
            }

            var entities2 = new List<Documentss>();
            for (int i = 1; i <= 10; i++)
            {
                var entity = new Documentss
                {
                    Id = Guid.NewGuid(),
                    DocumentBaseId = Guid.NewGuid(),
                    ActiveEndTime = DateTime.Now,
                    IsDeleted = false,
                    ConcurrencyToken="hdofasnhlrewhilhasefhwoae",
                    SysFileId=Guid.NewGuid(),
                    CreationTime =DateTime.Now,
                    CreatorUserId = 1,
                    CreatorUserName = "zhou",
                    DocumentRevision = "de"
                };
                entities2.Add(entity);
            }
            ...
            //}
            //var entities3 = new List<Documents>();
            var entities4 = new List<Documentss>();

            //context.BulkInsert(entities2);

            var dfs = context.Documentss.ToList();

            //var fd = context.Product.ToList();
            for (int i = 0; i < dfs.Count; i++)
            {
                dfs[i].DocumentRevision += "帅";
            }
            entities4.AddRange(dfs);
            ...
            //for (int i = 0; i <= context.Product.ToList().Count; i++)
            //{
            //    entities4.Add(context.Product.ToList()[i]);
            //    if (i > 5)
            //    {
            //        break;
            //    }
